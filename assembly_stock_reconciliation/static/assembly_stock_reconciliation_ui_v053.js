...
function ReconciliationPanel({ context }) {
  ...
  const noOp = itemPreview && Number(itemPreview.build_consumption_target ?? itemPreview.calculated_consumption ?? 0) === 0;
  ...
  return h("div", { style: styles.panel },
    ...
    h("div", { style: styles.card },
      h("h3", { style: { marginTop: 0 } }, "1. Allocation Review"),
      h("p", { style: { marginTop: 0, opacity: 0.78 } },
        "Shows Production Build Orders using this Part. Batch / serial is the primary stock identifier; the internal Stock Item ID is shown only as secondary reference. Correct any wrong allocations in InvenTree before reconciling."
      ),
      (stock.part_allocation_review || []).length === 0
        ? h(Alert, { kind: "warn" }, "This Part has no active allocations on Production Build Orders.")
        : h("div", { style: { overflowX: "auto" } },
            h("table", { style: { ...styles.table, tableLayout: "fixed", minWidth: "850px" } },
              h("colgroup", null,
                h("col", { style: { width: "13%" } }),
                h("col", { style: { width: "20%" } }),
                h("col", { style: { width: "12%" } }),
                h("col", { style: { width: "31%" } }),
                h("col", { style: { width: "16%" } }),
                h("col", { style: { width: "8%" } })
              ),
              h("thead", null, h("tr", null,
                h("th", { style: styles.th }, "Build Order"),
                h("th", { style: styles.th }, "Build Part"),
                h("th", { style: styles.th }, "Part Allocated"),
                h("th", { style: styles.th }, "Allocated Stock"),
                h("th", { style: styles.th }, "Current Stock Allocation"),
                h("th", { style: styles.th }, "Selected")
              )),
              h("tbody", null,
                ...(stock.part_allocation_review || []).map((row) =>
                  h("tr", { key: `part-allocation-${row.build}`, style: row.uses_current_stock_item ? { background: "rgba(25, 113, 194, 0.035)" } : undefined },
                    h("td", { style: { ...styles.td, verticalAlign: "top", fontWeight: 600 } }, row.reference),
                    h("td", { style: { ...styles.td, verticalAlign: "top", overflowWrap: "anywhere" } }, row.build_part || "—"),
                    h("td", { style: { ...styles.td, verticalAlign: "top" } }, fmt(row.part_allocated)),
                    h("td", { style: { ...styles.td, verticalAlign: "top" } },
                      ...((row.stock_items || []).map((si, idx) =>
                        h("div", {
                          key: `allocation-stock-${row.build}-${si.stock_item}`,
                          style: {
                            padding: idx === 0 ? "0 0 6px" : "6px 0",
                            borderTop: idx === 0 ? "none" : "1px solid rgba(128,128,128,0.16)"
                          }
                        },
                          h("div", { style: { fontWeight: si.current_stock_item ? 700 : 600, lineHeight: 1.35 } },
                            `${si.identifier || `Stock Item #${si.stock_item}`} — ${fmt(si.allocated)}`,
                            si.current_stock_item ? h("span", { style: { marginLeft: "6px" } }, "← current") : null
                          ),
                          (si.batch || si.serial)
                            ? h("div", { style: { opacity: 0.58, fontSize: "11px", marginTop: "1px" } }, `Stock Item #${si.stock_item}`)
                            : null
                        )
                      )),
                      row.multiple_stock_items
                        ? h("div", {
                            style: {
                              marginTop: "6px",
                              paddingTop: "6px",
                              borderTop: "1px solid rgba(240,140,0,0.35)",
                              fontWeight: 600,
                              color: "#f08c00",
                              lineHeight: 1.35
                            }
                          }, "⚠ Multiple stock batches / items allocated")
                        : null
                    ),
                    h("td", { style: { ...styles.td, verticalAlign: "top" } },
                      row.uses_current_stock_item
                        ? h("div", null,
                            h("div", { style: { fontWeight: 700 } }, `${stock.batch || stock.serial || `Stock Item #${stock.stock_item}`} — ${fmt(row.current_stock_item_allocated)}`),
                            (stock.batch || stock.serial)
                              ? h("div", { style: { opacity: 0.58, fontSize: "11px", marginTop: "1px" } }, `Stock Item #${stock.stock_item}`)
                              : null
                          )
                        : h("span", { style: { opacity: 0.6 } }, "—")
                    ),
                    h("td", { style: { ...styles.td, verticalAlign: "top", fontWeight: selectedBuilds.includes(row.build) ? 700 : 400 } },
                      selectedBuilds.includes(row.build) ? "Yes" : "No"
                    )
                  )
                )
              )
      ...
      (() => {
        const otherPartRows = (stock.part_allocation_review || []).filter((row) =>
          !row.uses_current_stock_item && !selectedBuilds.includes(row.build)
        );
        if (!otherPartRows.length) return null;
        return h("div", { style: { marginTop: "12px" } },
          h(Alert, { kind: "warn" },
            h("div", null,
              h("div", { style: { fontWeight: 700, marginBottom: "6px" } },
                `Other Production BOs use ${stock.part || "this Part"} from other stock batches / items:`
              ),
              ...otherPartRows.map((row) =>
                h("div", { key: `other-part-${row.build}`, style: { marginTop: "3px" } },
                  `${row.reference} — ${row.build_part || "—"}: ${fmt(row.part_allocated)}`
                )
              )
            )
          )
      ...
      (() => {
        const currentUnselected = (stock.part_allocation_review || []).filter((row) =>
          row.uses_current_stock_item && !selectedBuilds.includes(row.build)
        );
        if (!currentUnselected.length) return null;
        const currentIdentifier = stock.batch || stock.serial || `Stock Item #${stock.stock_item}`;
        return h("div", { style: { marginTop: "10px" } },
          h(Alert, { kind: "warn" },
            h("div", null,
              h("div", { style: { fontWeight: 700, marginBottom: "6px" } },
                `Current stock ${currentIdentifier} is allocated to unselected Production BOs:`
              ),
              ...currentUnselected.map((row) =>
                h("div", { key: `current-unselected-${row.build}`, style: { marginTop: "3px" } },
                  `${row.reference} — ${row.build_part || "—"}: ${fmt(row.current_stock_item_allocated)}`
                )
              ),
              h("div", { style: { marginTop: "7px", opacity: 0.82 } },
                "Investigate these allocations before reconciliation if they may be part of the same physical return."
              )
            )
          )
        );
    ...
    h("div", { style: styles.card },
      h("h3", { style: { marginTop: 0 } }, "2. Select relevant Build Orders"),
      h("p", { style: { marginTop: 0, opacity: 0.78 } },
        ...
        : h(React.Fragment, null,
            ...
            ...stock.builds.map((b) =>
              h("label", {
                ...
                h("span", null, b.build_part ? `— ${b.build_part}` : ""),
    ...
    h("div", { style: styles.card },
      h("h3", { style: { marginTop: 0 } }, "3. Enter physical return"),
    ...
    itemPreview ? h("div", { style: styles.card },
      h("h3", { style: { marginTop: 0 } }, "4. Review reconciliation"),
      h("div", { style: styles.grid },
        ...
        h(SummaryStat, { label: "Physical Quantity Delta", value: fmt(itemPreview.calculated_consumption) }),
        h(SummaryStat, { label: "Build Consumption to Record", value: fmt(itemPreview.build_consumption_target ?? itemPreview.calculated_consumption) }),
        h(SummaryStat, { label: "Inventory Reconciliation Adjustment", value: Number(itemPreview.inventory_reconciliation_adjustment || 0) > 0 ? `+${fmt(itemPreview.inventory_reconciliation_adjustment)}` : "0" }),
        ...
        h(SummaryStat, { label: "Actual Spillage / Overage", value: fmt(itemPreview.planned_spillage_consumed) }),
        ...
        h(SummaryStat, { label: "Spillage Distribution", value: (itemPreview.consumption_plan || []).filter((line) => Number(line.planned_spillage_consumed || 0) > 0).map((line) => `${line.build_reference}: ${fmt(line.planned_spillage_consumed)}`).join(" / ") || "0" }),
            ...
            h("table", { style: styles.table },
              h("thead", null,
                h("tr", null,
                  ...
                  h("th", { style: styles.th }, "Spillage / Overage"),
              ...
              h("tbody", null,
                ...itemPreview.consumption_plan.map((line, i) =>
                  h("tr", { key: `${line.build_item}-${i}` },
                    ...
                    h("td", { style: styles.td }, line.build_part || "—"),
                    h("td", { style: styles.td }, fmt(line.allocated)),
                    h("td", { style: styles.td }, fmt(line.planned_spillage_consumed)),
      ...
      hardWarning && !blocking
        ? h("div", { style: { ...styles.alertWarn, marginTop: "14px" } },
            h("strong", null,
              itemPreview.policy_classification === "above_spillage_allowance"
                ? "HARD WARNING — consumption exceeds approved allowance"
                : itemPreview.policy_classification === "below_nominal"
                  ? "HARD WARNING — physical return exceeds nominal expectation"
                  : "HARD WARNING — investigation and explicit approval required"
            ),
            h("p", null,
              itemPreview.policy_classification === "above_spillage_allowance"
                ? `Physical consumption is ${fmt(itemPreview.calculated_consumption)}, which exceeds nominal consumption of ${fmt(itemPreview.nominal_expected_consumption)} plus permitted spillage / overage of ${fmt(itemPreview.planned_spillage_allowance)}. An override will create ${fmt(itemPreview.exception_allocation_required)} of additional exception allocation and consume it against the selected Build Order(s). Do not override until the discrepancy has been investigated.`
                : itemPreview.policy_classification === "below_nominal"
                  ? `Physical consumption is ${fmt(itemPreview.calculated_consumption)}, below nominal selected-BO consumption of ${fmt(itemPreview.nominal_expected_consumption)}. An override will still record the nominal Build Order consumption and separately add back ${fmt(itemPreview.inventory_reconciliation_adjustment)} as a tracked inventory reconciliation adjustment so final stock matches the physical return. Do not override until the discrepancy has been investigated.`
                  : "This reconciliation falls outside the normal policy range. Do not override until the discrepancy has been investigated."
            ),
