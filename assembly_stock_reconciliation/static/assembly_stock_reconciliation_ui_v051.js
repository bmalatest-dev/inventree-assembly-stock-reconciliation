  ...
  return h("div", { style: styles.panel },
    ...
    h("div", { style: styles.card },
      ...
      h("p", { style: { marginTop: 0, opacity: 0.78 } },
        "This shows every Production Build Order where this Part is allocated across any Stock Item. It also identifies where this exact Stock Item is allocated. Correct any wrong allocations in InvenTree before reconciling."
      ),
        ...
        : h("div", { style: { overflowX: "auto" } },
            h("table", { style: styles.table },
              h("thead", null, h("tr", null,
                ...
                h("th", { style: styles.th }, "Allocated Stock (Batch / ID)"),
                h("th", { style: styles.th }, "Current Batch"),
                h("th", { style: styles.th }, "Selected")
              )),
              h("tbody", null,
                ...(stock.part_allocation_review || []).map((row) =>
                  h("tr", { key: `part-allocation-${row.build}` },
                    h("td", { style: styles.td }, row.reference),
                    h("td", { style: styles.td }, row.build_part || "—"),
                    h("td", { style: styles.td }, fmt(row.part_allocated)),
                    h("td", { style: styles.td },
                      ...(row.stock_items || []).map((si) =>
                        h("div", {
                          key: `allocation-stock-${row.build}-${si.stock_item}`,
                          style: {
                            marginBottom: "4px",
                            fontWeight: si.current_stock_item ? 700 : 400,
                            whiteSpace: "nowrap"
                          }
                        },
                          h("span", null, `${si.identifier || `Stock #${si.stock_item}`} — ${fmt(si.allocated)}`),
                          si.current_stock_item
                            ? h("span", { style: { marginLeft: "6px" } }, "← current")
                            : null,
                          (si.batch || si.serial)
                            ? h("span", { style: { marginLeft: "6px", opacity: 0.55, fontSize: "11px" } }, `(#${si.stock_item})`)
                            : null
                        )
                      ),
                      row.multiple_stock_items
                        ? h("div", { style: { marginTop: "6px", fontWeight: 600, color: "#f08c00" } },
                            "⚠ Multiple stock batches / items of this Part are allocated to this BO"
                          )
                        : null
                    ),
                    h("td", { style: styles.td }, row.uses_current_stock_item ? `${stock.batch || `#${stock.stock_item}`} (${fmt(row.current_stock_item_allocated)})` : "No"),
                    h("td", { style: styles.td }, selectedBuilds.includes(row.build) ? "Yes" : "No")
                  )
                )
              )
            )
          ),
      (stock.part_allocation_review || []).some((row) => !selectedBuilds.includes(row.build))
        ? h(Alert, { kind: "warn" },
            "This Part has active allocations on one or more Production Build Orders which are not selected. This is informational because those allocations may belong to other Stock Items."
          )
        : null,
      (stock.part_allocation_review || []).some((row) => row.uses_current_stock_item && !selectedBuilds.includes(row.build))
        ? h(Alert, { kind: "warn" },
            `Stock Item #${stock.stock_item} itself is allocated to one or more Production Build Orders which are not selected. Investigate before reconciliation.`
          )
        : null
    ),
